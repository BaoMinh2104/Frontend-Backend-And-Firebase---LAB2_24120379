...
def _caption_record_from_snapshot(snapshot):
    data = snapshot.to_dict() or {}
    return {
        "id": snapshot.id,
...
def _title_from_record(prompt: str | None, filename: str) -> str:
    source = _clean_title(prompt) or _clean_title(filename)
...
def _thumbnail_data_url(image_bytes: bytes) -> str | None:
    try:
...
def save_caption_record(
    ...
    doc_ref.set(
        {
            "filename": filename.strip() or "uploaded_image",
            "content_type": content_type,
            "prompt": prompt.strip() if prompt and prompt.strip() else None,
            "caption": cleaned_caption,
            "thumbnail_data_url": _thumbnail_data_url(image_bytes),
            "created_at": _now(),
        }
    )

    _conversation_ref(uid, conversation_id).update({"updated_at": _now()})
    return _caption_record_from_snapshot(doc_ref.get())


def load_caption_records(uid: str, conversation_id: str):
    docs = (
        _message_collection(uid, conversation_id)
        .order_by("created_at")
        .stream()
    )
    return [_caption_record_from_snapshot(doc) for doc in docs]


def maybe_generate_title_from_first_record(
    uid: str,
    conversation_id: str,
    prompt: str | None,
    filename: str,
    ...
    if not title:
        update_data["title"] = _title_from_record(prompt, filename)
